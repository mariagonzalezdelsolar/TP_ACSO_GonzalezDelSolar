...
int directory_findname(struct unixfilesystem *fs, const char *name, int dirinumber, struct direntv6 *dirEnt) {
  ...
  if (fs == NULL || name == NULL || dirEnt == NULL) return -1; // Error: puntero nulo
  
  struct inode dir_inode;

  // MANEJO DE ERRORES
  // Errores de inodos
  if (inode_iget(fs, dirinumber, &dir_inode) < 0) return -1; // Error al leer el inodo del directorio.
  
  if ((dir_inode.i_mode & IALLOC) == 0) return -1; // Inodo no asignado.

  // Errores de directorio
  if ((dir_inode.i_mode & IFDIR) == 0) return -1; // No es un directorio.

  char block[DISKIMG_SECTOR_SIZE];
  int num_blocks = (inode_getsize(&dir_inode) + DISKIMG_SECTOR_SIZE - 1) / DISKIMG_SECTOR_SIZE;
  ...
  for (int blk = 0; blk < num_blocks; blk++) {
    ...
    int num_entries = DISKIMG_SECTOR_SIZE / sizeof(struct direntv6);
