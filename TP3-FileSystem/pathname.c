...
int pathname_lookup(struct unixfilesystem *fs, const char *pathname) {
   ...
   if (pathname[0] != '/') return -1; // Error: path inválido.
    ...
    while (component != NULL) { // Recorro los componentes del path.
        ...
        if (strlen(component) > DIRENT_NAME_LEN) return -1; 

        int result = directory_findname(fs, component, current_inumber, &dir_entry);
        
        if (result < 0) return -1; // Error: Componente no encontrado o error de acceso.
